...
BEGIN
    ...
    IF NEW.start_year IS NOT NULL AND NEW.end_year IS NOT NULL AND NEW.start_year > NEW.end_year THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'start_year must be before end_year';
    END IF;

    -- start_year must be after 1900
    IF NEW.start_year < 1900 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'start_year must be after 1900';
    END IF;
    ...
    IF NEW.runtime_minutes IS NOT NULL AND (NEW.runtime_minutes < 0 OR NEW.runtime_minutes > 1440) THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'runtime_minutes must be between 0 and 1440';
    END IF;
