...
namespace SeedData.Handlers.Neo4j.Migrators
{
    public static class AttributesNeo4jMigrator
    {
        ...
        )
        {
            ...
            {
                var batchFromMySql = await mysqlContext.Attributes
                    .AsNoTracking()
                ...
                var neoBatch = batchFromMySql
                    .Select(MapAttributesEntity)
                    .ToList();

                await Neo4jAttributesMapper.UpsertAttributes(neoBatch, batchSize: 1000);

                Console.WriteLine(
                    $"Migrated page {pageIndex + 1} with {neoBatch.Count} attributes to Neo4j...");
