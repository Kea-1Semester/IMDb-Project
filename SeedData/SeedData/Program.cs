...
internal static class Program
{
    private static void Main(string[] args)
    {
        ...
        var dbContextOptions = new DbContextOptionsBuilder<ImdbContext>().UseMySql(Env.GetString("ConnectionString"),
            ServerVersion.AutoDetect(Env.GetString("ConnectionString"))).Options;
        ...
        using (var context = new ImdbContext(dbContextOptions))
        {
            ...
            AddPersonToDb.AddPerson(context, nameBasicPath, 100000);
            AddCrewToDb.AddCrew(context, titleCrewPath, 5000);
            AddEpisode.AddEpisodes(context, titleEpisodePath, 50000);
            AddActor.AddActorToDb(context, titlePrincipalsPath);
            AddRating.AddRatingToDb(context, titleRatingsPath);
            AddAkas.AddAkasToDb(context, titleAkasPath, 50000);
